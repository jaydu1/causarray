...
def comp_score(Y, CF, celltype, Z, W_hat=None):
    ...
    if W_hat is None:
        ...
        adata.obsm['X_emb'] = W_hat
        embed = 'X_emb'

    scib.me.cluster_optimal_resolution(adata, cluster_key="cluster", label_key="celltype", verbose=False)

    # ARI on response space
    ARI = scib.metrics.ari(adata, cluster_key="cluster", label_key="celltype")
    
    # ASW on latent sapce
    ASW = scib.me.silhouette(adata, label_key="celltype", embed=embed)
    ...
    #     verbose=False,
    # )
    # adata_pre = sc.AnnData(Y.copy())
    # sc.pp.log1p(adata_pre)
    # adata_post = sc.AnnData(CF.copy())
    # PCR = pcr_comparison(adata_pre,  adata_post, 
    #     covariate_pre=Z, covariate_post=Z,# embed='X_pca', n_comps=100
    #     )
...
def pcr_comparison(
    ...
    covariate_pre, covariate_post, embed=None, n_comps=50, scale=True, verbose=False
):
    ...
    **Examples**
    ...
    pcr_after = pcr(
        adata_post,
        covariate=covariate_post,
        embed=embed,
        recompute_pca=True,
        n_comps=n_comps,
        verbose=verbose,
    )
    print(pcr_before, pcr_after)
    if scale:
...
def pcr(adata, covariate, embed=None, n_comps=50, recompute_pca=True, verbose=False):
    ...
    **Examples**
    ...
    covariate_values = covariate# adata.obs[covariate]
...
def pc_regression(
    ...
    for i in range(n_comps):
        ...
        r2_score = np.abs(lm.score(covariate, pc))
        r2.append(r2_score)
